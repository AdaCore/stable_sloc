...
fail_if_not_equal(
    "Unexpected entry load failures",
    0,
    len(match_res.load_diagnostics)
)

fail_if_not_equal(
    "Unexpected number of match results",
    3,
    len(match_res.match_results)
)
...
for res in match_res.match_results:
    if res.success:
        fail_if_not_equal(
            "match success on unexpected file",
            "pkg.adb",
            res.file
        )
        success_count += 1
