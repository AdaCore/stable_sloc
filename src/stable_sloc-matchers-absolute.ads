...
package Stable_Sloc.Matchers.Absolute is
   ...
   overriding
   function Dump_Spec (Self : Absolute_Matcher) return TOML.TOML_Value;
   --  Return the text that can be used to re-create Self.

   overriding
   function Image (Self : Absolute_Matcher) return Unbounded_String;
...
private
   ...
   type Absolute_Matcher is new Sloc_Matcher_T with record
      Span : Sloc_Span;
