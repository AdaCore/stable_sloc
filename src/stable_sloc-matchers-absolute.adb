...
package body Stable_Sloc.Matchers.Absolute is
   ...
   overriding
   function Match
     (Self : Absolute_Matcher; File : Virtual_File) return Sloc_Match_Vec
   is
      use GNATCOLL.Utils;
      use type GNAT.Strings.String_Access;
      Buffer_Acc : GNAT.Strings.String_Access := GNATCOLL.VFS.Read_File (File);
         ...
         if Actual < Self.Span.Start_Sloc.Column then
            Res :=
              (False,
               Reason =>
                 +"Line"
                 & Self.Span.Start_Sloc.Line'Image
                 & " of "
                 & File.Display_Full_Name
   ...
   overriding
   function Dump_Spec (Self : Absolute_Matcher) return TOML.TOML_Value is
   ...
   overriding
   function Image (Self : Absolute_Matcher) return Unbounded_String is
   begin
