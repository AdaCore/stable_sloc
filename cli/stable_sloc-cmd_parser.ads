...
package Stable_Sloc.Cmd_Parser is
   ...
   function Str_To_File (Filename : String) return GNATCOLL.VFS.Virtual_File
   is (GNATCOLL.VFS.Create (GNATCOLL.VFS."+" (Filename)));
   ...
   Parser : Argument_Parser :=
     Create_Argument_Parser (Help => "Stable_Sloc CLI interface");

   package Specs is new
     Parse_Option_List
       (Parser,
        Short      => "-s",
        Long       => "--spec",
   ...
   package Output is new
     Parse_Option
       (Parser,
        ...
        Long        => "--output",
        Help        =>
          "File to which the loaded entries will be written at"
   ...
   package Update_Requests is new
     Parse_Option_List
       (Parser,
        Short      => "-u",
        Long       => "--update",
        Usage_Text =>
          "[--update|-u IDENTIFIER:KIND:FILENAME:START_LINE"
